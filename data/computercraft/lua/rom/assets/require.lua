...
_G.package.path = table.concat({
    ...
    "/lib/?.lua",
    "/lib/?/init.lua",
   "rom/modules/lib/?.lua", -- added minerbadd, dropped apis
-- _G.Muse.require or  "rom/modules/lib/?.lua
    --[[
    "/rom/apis/?",
    "/rom/apis/?.lua",
    "/rom/apis/?/init.lua",
...
_G.package.preload = {}
---@diagnostic disable-next-line: duplicate-set-field
_G.package.seeall = function() error("NotImplemented: package.seeall") end
_G.module = function() error("NotImplemented: module") end
...
local _package_path_loader = function(name)
    ...
    for pattern in package.path:gmatch("[^;]+") do
        ...
        if exists(fpath) and not isDir(fpath) then -- minerbadd no fs
...
_G.package.loaders = {
    ...
    function(name)
        ...
        for pattern in package.path:gmatch("[^;]+") do
            ...
            local fpath = pattern:gsub("%?", fname)
            if exists(fpath) and not isDir(fpath) then
