...
local text = "Beginning "..testName.." at "..move.ats()
local tester = check.open(_G.Muse.checks, "muse", testName, text) -- the test object
local function checking(partID, note, result) tester.part(testName, partID, note, result) end -- for test results
...
for code, remaining, at, direction, all in step.to({105, 156, 207}) do stepping = stepping + 0.001
  tester.message(stepping, code, remaining, core.string(at), direction, all)
end  
checking(28, "stepped to", core.ats())

tester.close("Test "..testName.." complete") 
