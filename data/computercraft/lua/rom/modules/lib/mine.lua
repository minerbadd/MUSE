...
local function atWork(plan, head, level, key, value) 
  ...
  core.report(3, "mine at work", plan.name, pathDistance, #pathOperations)
...
local function mineHead(mineheadName) -- move to minehead
  ...
  core.report(5, "mine", "At", mineheadName)
...
function _mine.down(mineheadName, targetLevel, shaftPlansDown, shaftPlansName) 
  ...
  mineHead(mineheadName); for level = 0, targetLevel do -- marks minehead 00
    ...
    core.report(5, "mine", "down", mineheadName, plan.name, "plan", planResult, "at level", level)
  ...
  core.report(4, "mine", "down", mineheadName, plan.name, "plan"..planResult.."done at level", targetLevel + 1)
...
function _mine.back(mineheadName, targetLevel, shaftPlansBack, shaftPlansName) 
  ...
  local workResult, workReport; for level = targetLevel + 1, 1, -1 do -- end up shaft from level 1 to 0
    ...
    core.report(4, "mine", mineheadName.." "..plan.name.." level "..level.." "..move.ats().." "..workResult)  
...
function _mine.shaftOp(mineheadName, levels, shaftPlans) 
  ...
  core.message(2, "Inventory digging down ", downInventory, downResult, downReport)
  ...
  core.message(2, "Inventory digging back ", downInventory)
...
local function changeLevel(directionPlans, direction, thisLevel, targetLevel)
  ...
  for level = thisLevel, finallyFrom, movement do -- each level to next in appropriate direction
    core.report(4, "mine change level", level, finallyFrom, movement)
  ...
  end; core.report(4, "mine change level done", result, report)
...
local function checkLevel(markerName, borePlans, shaftPlans, markers)
  ...
  core.report(3, "mine check level", thisLevel, thisTag, "in", thisShaft)
  ...
  if not (higher or lower) then core.report(3, "mine check level at", thisLevel, markerName)
...
function _mine.toPost(markerName, borePlans, shaftPlans) 
  ...
  core.report(3, "mine", "level", levelDone, levelReport)
