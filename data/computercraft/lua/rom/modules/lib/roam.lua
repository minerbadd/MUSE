...
local function toAxes(change, currentAxes, targetAxes) 
  ...
  for i in ipairs(change) do targets[change[i]] =  targetAxes[change[i]] 
    ...
    core.report(5, "roam axes", change, currentAxes, targetAxes) -- protected call: `move.to` throws errors
...
local function permuting(currentAxes, targetAxes) 
  ...
  core.report(3, "roam retry", distance, mx, my, mz) -- if no movement, just return failure, else recur
