...
local pickMove = { -- t* turtle, tt* target
  function (_, hy, tz, ttx, tty, ttz) to(ttx, hy, tz); to(ttx, hy, ttz); to(ttx, tty, ttz) end, -- x, z, y
  function (tx, hy, _, ttx, tty, ttz) to(tx, hy, ttz); to(ttx, hy, ttz); to(ttx, tty, ttz) end, -- z, x, y
  function (_, hy, tz, ttx, tty, ttz) to(ttx, hy, tz); to(ttx, tty, tz); to(ttx, tty, ttz) end, -- x, y, z
  function (tx, hy, _, ttx, tty, ttz) to(tx, hy, ttz); to(tx, tty, ttz); to(ttx, tty, ttz) end, -- z, y, x
}; 
...
local function picks() 
  roam.picks.come = (roam.picks.come + 1) % #pickMove; core.status(3, "roam pickCome", roam.picks.come)
...
function roam.come(xyz) -- **needs GPS for {xyz}**, lib/remote RPC "come" dispatched by lib/net 
  ...
  local tty = py - 1; local hy = math.max(ty, tty) -- go high first
  local distance = math.abs(ttx-tx) + math.abs(tty-ty) + math.abs(ttz-tz) 
  local fuel = turtle.fuel(); local fuelOK = fuel > distance
  if not fuelOK then return
    "roam: Insufficient fuel ("..fuel..") for "..distance.." blocks to {"..ttx..","..tty..","..ttz.."}" 
  end
  local moveOK, report = core.pass(pcall(moveHere, tx, hy, tz, ttx, tty, ttz)) 
  if moveOK then return "At "..core.xyzf({ttx, tty, ttz}).."\nFuel: "..fuel end
...
```Lua
--]]
local firstDirection = {"x", "y", "z"}; -- first try y

local function firsts() 
  roam.picks.try = (roam.picks.try + 1) % #firstDirection; return firstDirection[roam.picks.try + 1] -- to try each in turn
end 

local op = {move = move.to, moves = moves.to}
...
local function tryTo(arguments) -- repeated calls try each direction in turn
  ...
  local _, x, y, z, facing = table.unpack(arguments); 
  local to = tonumber(x) and {tonumber(x), tonumber(y), tonumber(z), facing or "south"} or x
  local library = tonumber(x) and "move" or (type(x) == "string" and "moves" or error("roam.tryTo: Bad argument"))
  local moveOK, code, index, xyzfacing =  core.pass(pcall(op[library], to, firsts()))
...
local function go(line) -- returns the result from last chain command attempted
  ...
  local commandOK, code, remaining, xyzfacing -- outside of `for` for `return`
  local commands = chain(2, line, {}); for _, command in ipairs(commands) do -- line[1] is  "go"
    commandOK, code, remaining, xyzfacing = core.pass(pcall(doChain, command))
...
function roam.op(arguments) --:: roam.op(arguments: ":"[]) -> _Move turtle:_ -> `":" &:`
  local from = core.ats(); turtle.block(turtle.blocking())-- try motion and block if blocking has been enabled for testing
