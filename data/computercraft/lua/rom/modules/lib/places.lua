...
```
As might be guessed, this module deals with the idea of a `place`, a data stucture representing a position in the Minecraft coordinate system. With the libraries generated by this module, we can note the position of a turtle or the player's pocket computer, give a name and a descriptive label to that position (creating a `place`), recall that place by name, forget about a place, find distances between places, and find the places near (and nearest) a turtle or other places. We can `move` (and `step`) a turtle to such places by name. The `lib/place` module also introduces the idea of a `trail`, a sequence of turtle positions built on `lib/motion` tracking. Turtles can be moved (and stepped) along such trails. The `place` module supports the notion of a `site`, a set of places local to some geography. There can be a number of sites in a Minecraft world.

As we've indicated, these libraries depends on `lib/motion`. Keeping its <a href="../../docs/lib/motion.html" target="_blank"> exports </a> in mind might be helpful in understanding this module.
