...
```
<a id="paths"></a> 
The `field.paths` function might be thought of as a utility for a _plan prototype_. The function uses `runElements` to figure out how to travel over all of a three dimensional rectangular solid with an efficient path. The details of how to do this are picky. Perhaps <a href="../../drawings/07Paths.pdf" target="_blank"> this drawing </a> will help.
```Lua
--]]
...
```
<a id="plan"></a> 
The `field.plan` function moves the turtle to the computed start position for the plot (including any offset) and loads the prototype plan to generate the _plan function_. It calls this function (step 5) with the `fieldParameters` (including the plot bound). The _plan function_ uses `field.paths` and the plot bound to compute the `start`, `even`, `odd`, and (optionally) `last` paths to fill in the rest of the information from the plan prototype (step 6) to produce a complete `plan` (as we have <a href="#plan" target="_blank"> discussed previously</a>). The computed paths define a "three dimensional ox plow" to minimize turtle motion in moving to every block in the three dimensional rectangular solid defined by the plot `bound`.
```Lua
--]]
...
local function home(xyz, result)
  local OK, failure = core.pass(pcall(turtle.digTo, xyz)); 
  return OK and result or error("field: can't go home: "..failure)
end
...
```
<a id="extents"></a> 
The `field.extents` function, a utility for _field functions_, defines a (virtual) coordinate system in terms of `stride` (default shorter) and `run` (default longer) axes. Defaults are overridden if the extent is `faced`. Each ``plot`` of a field is positioned relative to the next ``striding`` along the `stride` axis to encompass the specified `bounds`. The `striding` is determined by the size of the `stride` in the `strides` dictionary that is keyed by the name of the field operation, the `opName`. Each _field file_ defines its `plots` in terms of the `stride` and `run` coordinate system. The number of plots, `nplots` in the `bounds` is computed given the extent along the `stride` axis and the `stride` for a given field operation. An indication for the number of turtle `slots` expected to be involved in each field operation for each `plot` is also returned. 
