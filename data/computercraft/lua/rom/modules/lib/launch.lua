--[[
## Global Positioning System Launch: lib/launch
```md
--:! {launch: [":"]():} <- **GPS Deployment Command Line Library** -> muse/docs/lib/launch.md 
--:| launch: _Assemble, launch, and startup GPS computers._ -> launch, _launch
```
...
```Lua
--]]
local launch, _launch = {}, {}; launch.hints = {} ---@module "signs.launch"-- exports, internals for analysis, and hints
...
function _launch.actuate(commands)
  ...
  --:: `_launch.actuate(commands: [command: ":", direction: facing]) -> _Move to boot a GPS host and retrieve floppy._ -> ":"
...
local function check() -- make sure all required is available
  local modems = assert(turtle.find({modem}), "launch: need ender modem") 
  local disks = assert(turtle.find({drive}), "launch: need drive") -- select drive
  assert(turtle.find({floppy}), "launch: need floppy") -- select floppy
  assert(count(computer) >= 4, "launch: need 4 computers")
  assert(modems.count >= 4, "launch: need 4 modems")
  assert(disks.count >= 4, "launch: need 4 disks")
  assert(turtle.find(turtle.category("fuel")), "launch: need fuel")
end

local order = {"north", "south", "east", "west"} -- for efficient movement

function _launch.launch(commands) -- use `locate` to give a `place` a `location`
  --:: `_launch.launch(commands: [command: ":", location: ":", yD: #:?]) -> _Check before journey then launch._ -> ":"
  ...
  assert(location, "launch.launch: need to specify launch location")
  assert(place.match(location), "launch.launch: "..place.qualify(location).." not found")
...
local ops = {launch = _launch.launch, equip = _launch.equip, actuate = _launch.actuate, check = check} 

function launch.op(commands) 
  --:: launch.op(commands: ":"[]) -> _Command Line Interface._ -> `report: ":"` &:
  local ok, report = core.pass(pcall(ops[commands[1]], commands))
  if ok then return report else return "launch: "..report end -- report failure for error
end 
