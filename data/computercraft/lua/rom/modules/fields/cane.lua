...
The `plots` are defined so that `quarry` operates on the entire three block wide `slice` from `vW` to `vW+2` to let water into the `bottom` level of field. The dirt `layer` is at that same level but only on a two block wide part of the slice from `vW` to `vW+1`. The sand `cover` is one level up from the bottom on the same part of the slice. Cane is planted one level above that to `finish` the field. Finally, to `harvest` the cane, operations are done at the top of mature cane.
```Lua
--]]
...
local function finishOp(index) -- called by `field.make`
  local crops = {"minecraft:kelp", "minecraft:reeds", "minecraft:reeds"}; local crop = crops[(index % 3) + 1]
  local offsets = { {0, 0, 0}, {0, 1, 0}, {0, 1, 0} }; local offset = offsets[(index % 3) + 1]
...
local function harvestPath(index, plan) -- called by `field.make`; harvest one plot of cane field
  local offsets = { {0, 1, 0}, {0, 4, 0}, {0, 4, 0} }; local offset = offsets[(index % 3) + 1]
