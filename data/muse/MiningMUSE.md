...
<a id="task"/>
You've likely been driven to abstraction by what we've been going on (and on) about. Even so, the new and improved turtle is foundational in building the <a href="code/lib/task.html" target="_blank"> `lib/task` library</a>. It supports a CLI for low level turtle tasks specifying those abstracted directions (some, like `dig`, involving chewing gum while walking). The implementation for each of those low level turtle operations in `lib/task` uses the table accessed by the `direction` that was built for that operation by `lib/turtle`. Oh, the things a turtle made new can do.
